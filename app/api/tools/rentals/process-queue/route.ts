/**
 * POST /api/tools/rentals/process-queue?source=KHMER24|REALESTATE_KH
 *
 * Triggers the process-queue job for the specified source.
 * Admin-only.
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await processQueueJob(source as RentalSource);
